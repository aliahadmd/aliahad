...
export async function loadPersonalPost(slug) {
  try {
    const markdown = await import(`../assets/personal/${slug}.md?raw`)
...
export async function loadPersonalPosts() {
  const modules = import.meta.glob('../assets/personal/*.md', { eager: true })
  const posts = []

  for (const path in modules) {
    const slug = path.split('/').pop().replace('.md', '')
    const post = await loadPersonalPost(slug)
    if (post) {
      posts.push(post)
    }
  }
